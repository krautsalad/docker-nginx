...
[ -n "$DEFAULT_DOMAIN" ] && sed -i 's|ssl_certificate /etc/ssl/private/snakeoil.crt;|ssl_certificate /etc/nginx/ssl/'"$DEFAULT_DOMAIN"'.pem;|' /etc/nginx/sites-default/no-default.conf
