...
[ -n "$DEFAULT_DOMAIN" ] && sed -i 's|# ssl_stapling_file /etc/ssl/private/snakeoil.ocsp.der;|ssl_stapling_file /etc/nginx/ssl/'"$DEFAULT_DOMAIN"'.ocsp.der;|' /etc/nginx/sites-default/no-default.conf
[ "$DISABLE_IPV6" = "true" ] && sed -i -E 's/ ?\[[^]]+\]//g' /etc/nginx/h5bp/tls/ocsp_stapling.conf
